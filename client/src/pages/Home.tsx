...
// 第二章 转向
const TOPIC_DIALOGUES = new Set(['ageDialogue', 'educationDialogue', 'disabilityDialogue']);
...
const Chapter2Section = () => {
  ...
  const isStoryInView = useInView(storyRef, { amount: 0.3 });
  ...
  const handleNext = useCallback(() => {
    ...
    if (currentIndex < currentNodes.length - 1) {
      ...
      if (currentChapter === 'chapter1') {
        ...
        setCurrentIndex(0);
      } else if (TOPIC_DIALOGUES.has(currentChapter)) {
  ...
  }, [currentIndex, currentNodes, currentChapter, currentNode]);

  // Auto-advance narrator in Chapter 1, 2 and Fertility Dialogue
  useEffect(() => {
    // Only auto-advance if it's NOT the very last node (ending buttons)
    const isLastNode = currentIndex === currentNodes.length - 1;
    const shouldAutoAdvance = (currentChapter === 'chapter1' || currentChapter === 'chapter2' || currentChapter === 'fertilityDialogue' || currentChapter === 'chapter5') && 
        currentNode?.speaker === 'narrator' && 
        currentNode?.type === 'text' &&
        !isLastNode &&
        isStoryInView; // Only auto-advance when visible
  ...
  }, [currentChapter, currentIndex, currentNode, handleNext, isStoryInView]);
